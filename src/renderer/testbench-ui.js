/**
 * Model Testbench UI Template
 * Separated for clarity and maintainability
 */

// Testbench template
function getTestbenchTemplate() {
  return `
 ...
 <div class="card" style="margin-bottom: 20px;">
    ...
    <div class="card" style="margin-bottom:20px;">
      ...
      <div style="display: flex; gap: 12px; align-items: center;">
        <button class="btn btn-primary" id="btnRunTests" onclick="runTestbench()">
          🧪 Uruchom Testy
        </button>
        <button class="btn btn-secondary" id="btnCancelTests" onclick="cancelTestbench()" disabled style="background: var(--error);">
          ❌ Anuluj
        </button>
        <button class="btn btn-secondary" id="btnExportReport" onclick="exportTestbenchReport()" disabled>
...
async function initTestbenchView() {
  ...
  try {
    ...
    if (modelsResult.success) {
      ...
      document.getElementById('testbench-models-container').innerHTML =
        `<p style="color: var(--error);">Błąd: ${modelsResult.error}</p>`;
    }
  } catch (error) {
    document.getElementById('testbench-models-container').innerHTML =
      `<p style="color: var(--error);">Błąd ładowania modeli: ${error.message}</p>`;
  }
  ...
  try {
    ...
    if (scenariosResult.success) {
      ...
      document.getElementById('testbench-scenarios-container').innerHTML =
        `<p style="color: var(--error);">Błąd: ${scenariosResult.error}</p>`;
    }
  } catch (error) {
    document.getElementById('testbench-scenarios-container').innerHTML =
      `<p style="color: var(--error);">Błąd ładowania scenariuszy: ${error.message}</p>`;
  }
}
...
function renderTestbenchModels(models) {
  ...
  container.innerHTML = `
    <div style="margin-bottom: 10px;">
      <button class="btn btn-sm" onclick="selectAllModels(true)">✓ Zaznacz wszystkie</button>
      <button class="btn btn-sm" onclick="selectAllModels(false)" style="margin-left: 8px;">✗ Odznacz wszystkie</button>
    </div>
...
function renderTestbenchScenarios(scenarios) {
  ...
  container.innerHTML = `
    <div style="margin-bottom: 10px;">
      <button class="btn btn-sm" onclick="selectAllScenarios(true)">✓ Zaznacz wszystkie</button>
      <button class="btn btn-sm" onclick="selectAllScenarios(false)" style="margin-left: 8px;">✗ Odznacz wszystkie</button>
    </div>
...
async function runTestbench() {
  ...
  document.getElementById('btnCancelTests').disabled = false;
  document.getElementById('testbench-status').textContent = '⏳ Uruchamianie testów...';
  document.getElementById('testbench-progress').style.display = 'block';
  document.getElementById('testbench-results').style.display = 'none';

  addLog('info', `🧪 Rozpoczynam testy: ${selectedModels.length} modeli × ${selectedScenarios.length} scenariuszy`);

  // No polling needed - events come automatically via onTestbenchProgress
  ...
  try {
    ...
    if (result.success) {
      ...
      if (!window.testbenchState) window.testbenchState = {};
      ...
      document.getElementById('testbench-status').textContent = `❌ Błąd: ${result.error}`;
    }
  } catch (error) {
    addLog('error', `Błąd: ${error.message}`);
    document.getElementById('testbench-status').textContent = `❌ Błąd: ${error.message}`;
  } finally {
    document.getElementById('btnRunTests').disabled = false;
    document.getElementById('btnCancelTests').disabled = true;
    document.getElementById('testbench-progress').style.display = 'none';
  }
}

// Cancel running tests
async function cancelTestbench() {
  addLog('warn', 'Anulowanie testów...');
  document.getElementById('testbench-status').textContent = '⚠️ Anulowanie...';
  document.getElementById('btnCancelTests').disabled = true;
...
function updateTestbenchProgress(progress) {
  document.getElementById('testbench-progress-fill').style.width = progress.progressPercent + '%';
  document.getElementById('testbench-progress-fill').textContent = progress.progressPercent + '%';
  document.getElementById('testbench-current-model').textContent = progress.currentModel || '---';
  document.getElementById('testbench-current-scenario').textContent = progress.currentScenario || '---';
  document.getElementById('testbench-test-count').textContent = `${progress.completedTests}/${progress.totalTests}`;
}
...
function displayTestbenchResults(summary) {
  ...
  document.getElementById('testbench-results').style.display = 'block';
  document.getElementById('testbench-status').textContent = '✅ Testy zakończone';
}

// Export report
async function exportTestbenchReport() {
